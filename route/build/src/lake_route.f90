...
CONTAINS
  ...
  ! *********************************************************************
  SUBROUTINE lake_route(segIndex,   &    ! input: index of runoff reach to be processed
    ...
    if(nUps>0)then
      do iUps = 1,nUps
        ...
        q_upstream = q_upstream + RCHFLX_out(iRch_ups)%ROUTE(idxRoute)%REACH_Q
      end do
    endif
    ...
    if (verbose) then
      ...
      write(iulog,'(A,1X,G12.5)') 'precipitation [m3/s] = ', RCHFLX_out(segIndex)%basinprecip
      write(iulog,'(A,1X,G12.5)') 'evaporation [m3/s]   = ', RCHFLX_out(segIndex)%basinevapo
    end if
    ...
    if (iTime==1) then
      if ((is_vol_wm_jumpstart).and.(NETOPO_in(segIndex)%LakeTargVol)) then
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%REACH_WM_VOL ! update the initial condition with first target volume value
        ...
        if (isColdStart) then    ! Cold start ....... initialize flux structures
          select case(NETOPO_in(segIndex)%LakeModelType)
            case(endorheic)
              RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RPARAM_in(segIndex)%D03_S0 ! currently assumes all endorheic max storage are provided under doll formulation
            case(doll03)
              RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RPARAM_in(segIndex)%D03_MaxStorage
            case(hanasaki06)
              RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RPARAM_in(segIndex)%H06_Smax
            case(hype)
              RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = (RPARAM_in(segIndex)%HYP_E_emr - RPARAM_in(segIndex)%HYP_E_zero) * RPARAM_in(segIndex)%HYP_A_avg
            case default; ierr=20; message=trim(message)//'unable to identify the parametric lake model type'; return
          end select
        endif
      endif
    ...
    RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(0) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) ! updating storage at previous time step
    RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) + q_upstream * dt  ! input upstream discharge from m3/s to m3
    if ((LakeInputOption == 1) .or. (LakeInputOption == 2)) then ! considers runoff for lake fluxes
      RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) + RCHFLX_out(segIndex)%BASIN_QR(1) * dt ! add lateral flow
    end if
    if ((LakeInputOption == 0) .or. (LakeInputOption == 2)) then ! considers precipitation and evaporation for lake fluxes
      RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) + RCHFLX_out(segIndex)%basinprecip * dt ! input lake precipitation
      if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) > RCHFLX_out(segIndex)%basinevapo*dt) then ! enough water to evaporate
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RCHFLX_out(segIndex)%basinevapo * dt ! output lake evaporation
      else ! not enough water to evaporate (this cause water balance error in coupling mode)
        RCHFLX_out(segIndex)%basinevapo = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)/dt ! update basinevapo
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)=0._dp
      endif
    end if

    RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%REACH_WM_FLUX ! initialize actual water abstraction
    ...
    if((RCHFLX_out(segIndex)%REACH_WM_FLUX /= realMissing).and.(is_flux_wm)) then
      ...
      if (RCHFLX_out(segIndex)%REACH_WM_FLUX <= 0) then ! negative/injection
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RCHFLX_out(segIndex)%REACH_WM_FLUX*dt
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%REACH_WM_FLUX
      else ! positive/abstraction
        if (RCHFLX_out(segIndex)%REACH_WM_FLUX*dt <= RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)) then ! abstraction is smaller than water in the lake
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RCHFLX_out(segIndex)%REACH_WM_FLUX*dt
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%REACH_WM_FLUX
        else ! abstraction is larger than water in the river
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)/dt
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = 0._dp
        endif
      endif
    endif
    ...
    if (NETOPO_in(segIndex)%LakeTargVol) then ! The lake should follow the given target volume
      ...
      if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) < RCHFLX_out(segIndex)%REACH_WM_VOL) then ! The storage is smaller, accumulate
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = 0 ! lake/reservoir output is set to zero (0)
      else ! The storage is largrt, release to get to the target level
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q      = (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RCHFLX_out(segIndex)%REACH_WM_VOL) /dt
        RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%REACH_WM_VOL
      endif

    else ! if the lake is parameteric
      select case(NETOPO_in(segIndex)%LakeModelType)
        case(endorheic)
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = 0._dp ! no outflow
        case(doll03)
          ...
          if ((RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RPARAM_in(segIndex)%D03_S0) > 0) then
            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = RPARAM_in(segIndex)%D03_Coefficient * &
            (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RPARAM_in(segIndex)%D03_S0) * &
            ((RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RPARAM_in(segIndex)%D03_S0) / &
            ...
            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = 0
          endif
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q / secprday ! conversion to m**3 s**-1
          ! in case is the output volume is more than lake volume
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = min(RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q, RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)/dt)
          ! updating the storage
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q * dt
        ...
        case(hanasaki06)
          ...
          if (RPARAM_in(segIndex)%H06_I_mem_F) then ! if memeory is acive for this case then allocate the past input
            if (.not.allocated(RCHFLX_out(segIndex)%QPASTUP_IRF)) then
              past_length_I = floor(RPARAM_in(segIndex)%H06_I_mem_L * 31 * secprday / dt)
              allocate(RCHFLX_out(segIndex)%QPASTUP_IRF(12,past_length_I),stat=ierr)
              RCHFLX_out(segIndex)%QPASTUP_IRF( 1,:) = RPARAM_in(segIndex)%H06_I_Jan
              RCHFLX_out(segIndex)%QPASTUP_IRF( 2,:) = RPARAM_in(segIndex)%H06_I_Feb
              RCHFLX_out(segIndex)%QPASTUP_IRF( 3,:) = RPARAM_in(segIndex)%H06_I_Mar
              RCHFLX_out(segIndex)%QPASTUP_IRF( 4,:) = RPARAM_in(segIndex)%H06_I_Apr
              RCHFLX_out(segIndex)%QPASTUP_IRF( 5,:) = RPARAM_in(segIndex)%H06_I_May
              RCHFLX_out(segIndex)%QPASTUP_IRF( 6,:) = RPARAM_in(segIndex)%H06_I_Jun
              RCHFLX_out(segIndex)%QPASTUP_IRF( 7,:) = RPARAM_in(segIndex)%H06_I_Jul
              RCHFLX_out(segIndex)%QPASTUP_IRF( 8,:) = RPARAM_in(segIndex)%H06_I_Aug
              RCHFLX_out(segIndex)%QPASTUP_IRF( 9,:) = RPARAM_in(segIndex)%H06_I_Sep
              RCHFLX_out(segIndex)%QPASTUP_IRF(10,:) = RPARAM_in(segIndex)%H06_I_Oct
              RCHFLX_out(segIndex)%QPASTUP_IRF(11,:) = RPARAM_in(segIndex)%H06_I_Nov
              RCHFLX_out(segIndex)%QPASTUP_IRF(12,:) = RPARAM_in(segIndex)%H06_I_Dec
            else
              array_size = shape(RCHFLX_out(segIndex)%QPASTUP_IRF)
              past_length_I = array_size(2)
              RCHFLX_out(segIndex)%QPASTUP_IRF(simDatetime(1)%month(), 2:past_length_I) = RCHFLX_out(segIndex)%QPASTUP_IRF(simDatetime(1)%month(), 1:past_length_I-1) ! shift the memory
              RCHFLX_out(segIndex)%QPASTUP_IRF(simDatetime(1)%month(), 1) = q_upstream ! allocate the current qupstream
            endif
            ! mean and updating the inflow parameters
            past_length_I = floor(RPARAM_in(segIndex)%H06_I_mem_L * 31    * secprday / dt)
            RPARAM_in(segIndex)%H06_I_Jan = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 1,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Mar = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 3,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_May = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 5,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Jul = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 7,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Aug = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 8,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Oct = sum(RCHFLX_out(segIndex)%QPASTUP_IRF(10,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Dec = sum(RCHFLX_out(segIndex)%QPASTUP_IRF(12,1:past_length_I))/past_length_I
            past_length_I = floor(RPARAM_in(segIndex)%H06_I_mem_L * 30    * secprday / dt)
            RPARAM_in(segIndex)%H06_I_Apr = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 4,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Jun = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 6,1:past_length_I))/past_length_I
            RPARAM_in(segIndex)%H06_I_Sep = sum(RCHFLX_out(segIndex)%QPASTUP_IRF( 9,1:past_length_I))/past_length_I
            ...
            RPARAM_in(segIndex)%H06_I_Feb = SUM(RCHFLX_out(segIndex)%QPASTUP_IRF( 2,1:past_length_I))/past_length_I
          endif
          ...
          if ((RPARAM_in(segIndex)%H06_D_mem_F).and.(RCHFLX_out(segIndex)%REACH_WM_FLUX/=realMissing).and.(is_flux_wm)) then ! if memeory is acive for this case then allocate the past input
            if (RCHFLX_out(segIndex)%REACH_WM_FLUX<0) then ! demand cannot be negative for lake/reservoir
              RCHFLX_out(segIndex)%REACH_WM_FLUX = 0._dp
            endif
            if (.not.allocated(RCHFLX_out(segIndex)%DEMANDPAST_IRF)) then
              past_length_D = floor(RPARAM_in(segIndex)%H06_D_mem_L * 31 * secprday / dt)
              allocate(RCHFLX_out(segIndex)%DEMANDPAST_IRF(12,past_length_D),stat=ierr)
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 1,:) = RPARAM_in(segIndex)%H06_D_Jan
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 2,:) = RPARAM_in(segIndex)%H06_D_Feb
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 3,:) = RPARAM_in(segIndex)%H06_D_Mar
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 4,:) = RPARAM_in(segIndex)%H06_D_Apr
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 5,:) = RPARAM_in(segIndex)%H06_D_May
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 6,:) = RPARAM_in(segIndex)%H06_D_Jun
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 7,:) = RPARAM_in(segIndex)%H06_D_Jul
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 8,:) = RPARAM_in(segIndex)%H06_D_Aug
              RCHFLX_out(segIndex)%DEMANDPAST_IRF( 9,:) = RPARAM_in(segIndex)%H06_D_Sep
              RCHFLX_out(segIndex)%DEMANDPAST_IRF(10,:) = RPARAM_in(segIndex)%H06_D_Oct
              RCHFLX_out(segIndex)%DEMANDPAST_IRF(11,:) = RPARAM_in(segIndex)%H06_D_Nov
              RCHFLX_out(segIndex)%DEMANDPAST_IRF(12,:) = RPARAM_in(segIndex)%H06_D_Dec
            else
              array_size = shape(RCHFLX_out(segIndex)%DEMANDPAST_IRF)
              past_length_D = array_size(2)
              RCHFLX_out(segIndex)%DEMANDPAST_IRF(simDatetime(1)%month(), 2:past_length_D) = RCHFLX_out(segIndex)%DEMANDPAST_IRF(simDatetime(1)%month(), 1:past_length_D-1) ! shift the memory
              RCHFLX_out(segIndex)%DEMANDPAST_IRF(simDatetime(1)%month(), 1) = RCHFLX_out(segIndex)%REACH_WM_FLUX ! allocate the current demand
            endif
            ! mean and updating the demand parameters
            past_length_D = floor(RPARAM_in(segIndex)%H06_D_mem_L * 31    * secprday / dt)
            RPARAM_in(segIndex)%H06_D_Jan = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 1,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Mar = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 3,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_May = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 5,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Jul = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 7,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Aug = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 8,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Oct = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF(10,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Dec = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF(12,1:past_length_D))/past_length_D
            past_length_D = floor(RPARAM_in(segIndex)%H06_D_mem_L * 30    * secprday / dt)
            RPARAM_in(segIndex)%H06_D_Apr = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 4,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Jun = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 6,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Sep = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 9,1:past_length_D))/past_length_D
            RPARAM_in(segIndex)%H06_D_Nov = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF(11,1:past_length_D))/past_length_D
            select case(trim(calendar))
              case('noleap','365_day')
            ...
            RPARAM_in(segIndex)%H06_D_Feb = sum(RCHFLX_out(segIndex)%DEMANDPAST_IRF( 2,1:past_length_D))/past_length_D
          endif
          ...
          if (simDatetime(1)%month() == start_month .AND. simDatetime(1)%day() == 1 ) then
             RPARAM_in(segIndex)%H06_E_rel_ini = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) / (RPARAM_in(segIndex)%H06_alpha * RPARAM_in(segIndex)%H06_Smax)
          endif
          ...
          if (c >= RPARAM_in(segIndex)%H06_c_compare) then ! multi-year reservoir
            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = target_r * RPARAM_in(segIndex)%H06_E_rel_ini
          else if (0 <= c .AND. c < RPARAM_in(segIndex)%H06_c_compare) then
            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = RPARAM_in(segIndex)%H06_E_rel_ini * target_r * (c / RPARAM_in(segIndex)%H06_denominator)**RPARAM_in(segIndex)%H06_exponent  + &
          ...
          if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) < (RPARAM_in(segIndex)%H06_Smax * RPARAM_in(segIndex)%H06_frac_Sdead)) then
            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q - (RPARAM_in(segIndex)%H06_Smax * RPARAM_in(segIndex)%H06_frac_Sdead - RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) )/dt
            ! set negative outflow to zero
            if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q<0) then
                RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q=0
            end if

          ! Account for spil overflow if reservoir is completely filled.
          else if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) > RPARAM_in(segIndex)%H06_Smax) then
            RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q + (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RPARAM_in(segIndex)%H06_Smax)/ dt
          end if
        ...
        case(hype)
          ...
          RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_ELE = RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) / RPARAM_in(segIndex)%HYP_A_avg &
          ...
          F_lin = min(max((RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_ELE-RPARAM_in(segIndex)%HYP_E_min)/(RPARAM_in(segIndex)%HYP_E_lim-RPARAM_in(segIndex)%HYP_E_min),0._dp),1._dp)
          ...
          ! Q_spill
          Q_spill = 0._dp
          if (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_ELE > RPARAM_in(segIndex)%HYP_E_emr) then
...
!    WB = q_upstream * dt + RCHFLX_out(segIndex)%basinprecip * dt - RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q * dt &
!    - RCHFLX_out(segIndex)%basinevapo * dt - RCHFLX_out(segIndex)%REACH_WM_FLUX_actual * dt &
!    - (RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1) - RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(0))
!
...
!      write(iulog,'(A,1PG15.7)') 'dS [m3]          = ', RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)-RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(0)
!      write(iulog,'(A,1PG15.7)') 'S [m3]           = ', RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_VOL(1)
!      write(iulog,'(A,1PG15.7)') 'inflow [m3]      = ', q_upstream*dt
!      write(iulog,'(A,1PG15.7)') 'outflow [m3]     = ', RCHFLX_out(segIndex)%ROUTE(idxRoute)%REACH_Q*dt
!      write(iulog,'(A,1PG15.7)') 'precip [m3]      = ', RCHFLX_out(segIndex)%basinprecip*dt
!      write(iulog,'(A,1PG15.7)') 'evaporation [m3] = ', RCHFLX_out(segIndex)%basinevapo*dt
!      if (is_flux_wm) then
!        write(iulog,'(A,1PG15.7)') 'actual abstraction [m3] = ', RCHFLX_out(segIndex)%REACH_WM_FLUX_actual
!      endif
!      if (1<WB) then
