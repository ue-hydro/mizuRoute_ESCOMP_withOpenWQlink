...
CONTAINS
 ...
 RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%REACH_WM_FLUX ! initialize actual water abstraction

 ! update volume at previous time step
 RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(0) = RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(1)
 ...
 if (nUps>0) then ! this hru is not headwater
   ...
   do iUps = 1,nUps
     ...
     Qupstream = Qupstream + RCHFLX_out(iRch_ups)%ROUTE(idxDW)%REACH_Q
   end do
 ...
 RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_INFLOW = Qupstream ! total inflow from the upstream reaches
 ...
 if((RCHFLX_out(segIndex)%REACH_WM_FLUX /= realMissing).and.(is_flux_wm)) then
   if (Qabs > 0) then ! positive == abstraction
     if (RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(1)/dt > Qabs) then ! take out all abstraction from strorage
       RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(1) - Qabs*dt
     else ! if inital abstraction is greater than volume
       Qabs = Qabs - RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(1)/dt ! get residual Qabs after extracting from strorage
       RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_VOL(1) = 0._dp ! voluem gets 0
       if (Qupstream > Qabs) then ! then take out all residual abstraction from upstream inflow
         ...
         if (Qlat > Qabs) then ! then take residual abstraction out from lateral flow
           ...
           RCHFLX_out(segIndex)%ROUTE(idxDW)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%REACH_WM_FLUX - Qabs
         end if
       end if
     end if
   ...
   if (nUps>0) then
     do iUps = 1,nUps
       ...
       write(iulog,'(A,1X,I12,1X,G15.4)') ' UREACHK, uprflux=',NETOPO_in(segIndex)%UREACHK(iUps), &
             RCHFLX_out(iRch_ups)%ROUTE(idxDW)%REACH_Q
     enddo
   end if
 ...
 call diffusive_wave(segIndex,RPARAM_in(segIndex),                     &  ! input: parameter at segIndex reach
                     ...
                     RCHSTA_out(segIndex)%DW_ROUTE,           &  ! inout:
 ...
 if (qmodOption==0) then ! check reach water balance only if data assimilation is off
   call comp_reach_wb(NETOPO_in(segIndex)%REACHID, idxDW, Qupstream, Qlat, RCHFLX_out(segIndex), verbose, lakeFlag=.false.)
 end if
