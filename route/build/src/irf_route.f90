...
! print*, message
 ...
 if(NETOPO_in(segIndex)%REACHID == desireId) verbose = .true.
  ...
  RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(0) = RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(1)
  ...
  RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_INFLOW = q_upstream ! total inflow from the upstream reaches
  ...
  if((RCHFLX_out(segIndex)%REACH_WM_FLUX /= realMissing).and.(is_flux_wm)) then
    if (Qabs > 0) then ! positive == abstraction
      if (RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(1)/dt > Qabs) then ! take out all abstraction from strorage
        RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(1) = RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(1) - Qabs*dt
      else ! if inital abstraction is greater than volume
        Qabs = Qabs - RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(1)/dt ! get residual Qabs after extracting from strorage
        RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_VOL(1) = 0._dp ! volume gets 0
        if (q_upstream > Qabs) then ! then take out all residual abstraction from upstream inflow
          ...
          if (Qlat > Qabs) then ! then take residual abstraction out from lateral flow
            ...
            RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_WM_FLUX_actual = RCHFLX_out(segIndex)%REACH_WM_FLUX - Qabs
          end if
        end if
      end if
...
!    RCHFLX_out(segIndex)%ROUTE(idxIRF)%REACH_Q = 0._dp
  ...
  if (qmodOption==0) then ! check reach water balance only if data assimilation is off
    call comp_reach_wb(NETOPO_in(segIndex)%REACHID, idxIRF, q_upstream, Qlat, RCHFLX_out(segIndex), verbose, lakeFlag=.false.)
  end if
